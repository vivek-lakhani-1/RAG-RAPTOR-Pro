...
def build_vectorstore(tree_results: Dict[int, pd.DataFrame], embeddings, save_path="Vector_DB"):
    """Build and save a FAISS vectorstore from all texts in the RAPTOR tree."""
    all_texts = []
    all_embeddings = []
    all_metadatas = []
...
class RAPTORMethod:
    ...
    def run(self, query: str, k: int = 3) -> Dict[str, Any]:
        ...
        vectorstore = load_or_build_vectorstore(self.tree_results, self.embeddings)
...
def load_or_build_vectorstore(tree_results: Dict[int, pd.DataFrame], embeddings, save_path="faiss_index"):
    if os.path.exists(f"{save_path}/index.faiss"):
        ...
        logging.info("FAISS index not found. Building new one...")
        return build_vectorstore(tree_results, embeddings, save_path)
